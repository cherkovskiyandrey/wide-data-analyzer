...
/**
 * TODO: пока сделать так: ApplicationContext вытащить в public api, т.е. будет возможность и через @ServiceInject
 * и через getImpl. Как следствие грузим бандлы динамически.
 * Так же нужно написать стартер, которому передаём путь к конфигу со списком бандлов для запуска по политике LAZY или EAGER
 * В каждом бандле в манифест файле перечислены:
 * - api зависимости (в дефолтный класс лоадер) JAR HELL IS PASSABLE - thinking...
 * - общие библиотеки (в дефолтный класс лоадер) JAR HELL IS PASSABLE - thinking...
 * - сторонние библиотеки (в дефолтный класс лоадер) JAR HELL IS PASSABLE - thinking...
 * <p>
 * для их предварительной проверки наличия при анализе всех бандлов и при старте их.
 * А так же сервисы которые он выставляет.
 * Сервисы которые ему нужны не ясны на этапе запуска и если во время работы они потребуются а их не будет
 * выкинется ошибка.
 * <p>
 ...
 * - поддержка загрузки бандлов из базы
